...
本课是教程的完整 REPL，但有两类功能未集成：

1. **MCP 动态工具发现**（第 5 课 File 4）— 本课工具仍为硬编码。进阶练习：把 `TOOLS` 列表替换为 MCP 动态发现的工具。
2. **RAG 和 Skills**（第 10 课 File 3-4）— 进阶练习：把 `search_knowledge_base()` 或 `list_skills()` 集成到 REPL 中。
