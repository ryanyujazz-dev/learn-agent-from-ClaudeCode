...
从第 6 课开始，agent 需要**执行 bash 命令、读写文件**——这些操作都是 I/O 等待。如果用同步写法，执行一条命令时整个程序会卡住，无法同时处理流式输出。

async 是后续所有课程的基础，现在切换是为了后面不再分心。
...
修改 `async_chat.py`，在对话开始前用 `asyncio.gather()` 同时发两个不同问题给 LLM，打印两个回复，观察总耗时是否比串行快。
